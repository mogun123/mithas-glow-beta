...
class RealtimeService {
  ...
  subscribeToPresence(
    ...
    if (callbacks.onSync) {
      channel.on("presence", { event: "sync" }, () => {
        callbacks.onSync?.(channel.presenceState() as Record<string, PresencePayload[]>)
      })
    }
  ...
  // Subscribe to reel engagement (likes, comments)
  ...
  subscribeToNotifications(userId: string, onNotification: RealtimeCallback<Notification>): () => void {
    return this.subscribeToTable<Notification>(
      "notifications",
      "INSERT",
