...
export class HuggingFaceStableDiffusionService {
  ...
  private drawEyeLandmarks(ctx: CanvasRenderingContext2D, landmarks: any[]) {
    ...
    // Right eye landmarks  
    const rightEyeIndices = [362, 398, 384, 385, 386, 387, 388, 466, 263, 249, 390, 373, 380, 381, 382];
  ...
  private drawLipLandmarks(ctx: CanvasRenderingContext2D, landmarks: any[]) {
    ...
    // Outer lip landmarks
    const outerLipIndices = [61, 84, 17, 314, 405, 291, 375, 321, 308, 324, 318];

    // Inner lip landmarks
    const innerLipIndices = [78, 191, 80, 81, 82, 13, 312, 311, 310, 415, 308];
  ...
  async batchGenerateTryOn(
    ...
    const results = await Promise.allSettled(
      styles.map(style => 
        this.generateVirtualTryOn(basePrompt, faceLandmarks, sourceImage, style)
      )
    );
