...
const getEnv = (key: string, required = false): string => {
  ...
  // Vite environment variables (primary for this app)
  ...
  // Fallback to process.env if import.meta not available
  if (!value && typeof process !== "undefined" && process.env) {
    value = process.env[`NEXT_PUBLIC_${key}`] || process.env[key] || ""
  }

  if (required && !value) {
    console.error(`[ENV CRITICAL] Missing required environment variable: ${key}. This will cause the app to fail!`)
    console.error(`[ENV] Please create a .env file with VITE_${key}=your_value`)
  }
