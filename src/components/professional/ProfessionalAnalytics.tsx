...
export default function ProfessionalAnalytics({ artistId, onBack }: ProfessionalAnalyticsProps) {
  ...
  if (errorMessage) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4 text-center">
        ...
        <button onClick={() => void loadAnalytics()} className="mt-4 rounded-full bg-gradient-to-r from-pink-500 to-purple-600 px-4 py-2 text-xs font-black uppercase tracking-widest text-white shadow-lg shadow-pink-500/30">
          Retry
        </button>
      </div>
    );
  ...
  if (loading) {
    return (
      <div className="animate-in fade-in duration-500 min-h-[60vh] flex flex-col items-center justify-center bg-[#1a0b2e]">
        ...
        <p className="text-pink-300/70 font-black tracking-widest text-[10px] uppercase animate-pulse">Processing Data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="pb-32 animate-in fade-in slide-in-from-bottom-4 duration-500 bg-[#1a0b2e] min-h-screen">
      
      {/* 💎 Floating Glass Controls */}
      <div className="flex items-center justify-between mb-6 bg-[#2d1b4e]/80 backdrop-blur-2xl p-2 pl-5 rounded-[2rem] border border-pink-500/20 shadow-[0_8px_30px_rgba(0,0,0,0.3)]">
        <div>
          <h2 className="text-[10px] font-black text-pink-100 uppercase tracking-widest">Revenue</h2>
        </div>
        <div className="flex gap-1 p-1 bg-[#1a0b2e]/50 rounded-full border border-pink-500/20">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${
                timeRange === range
                  ...
                  : 'text-pink-300/70 hover:bg-[#2d1b4e]'
              }`}
            >
              {range}
            </button>
      ...
      <div className="relative bg-gradient-to-br from-[#2d1b4e] via-[#1e0f3a] to-[#2d1b4e] p-6 rounded-[2rem] shadow-2xl shadow-purple-900/20 mb-6 overflow-hidden border border-pink-500/20">
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-xl backdrop-blur-md border border-pink-500/20">
          ...
          <div className="flex items-center gap-2 text-[10px] font-extrabold tracking-wide text-pink-300/60 bg-black/20 w-fit px-3 py-1.5 rounded-lg border border-white/5">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        {/* Total Bookings */}
        <div className="bg-[#2d1b4e]/70 backdrop-blur-2xl p-5 rounded-3xl border border-pink-500/20 shadow-[0_8px_30px_rgba(0,0,0,0.3)] relative overflow-hidden group">
          ...
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white mb-3 shadow-lg shadow-purple-500/30">
            <Calendar className="w-5 h-5" />
          </div>
          <p className="text-3xl font-black text-pink-100 tracking-tight">{analytics.totalBookings}</p>
          <p className="text-[9px] font-extrabold uppercase tracking-widest text-pink-300/50 mt-1">Total Bookings</p>
        </div>
        ...
        <div className="bg-[#2d1b4e]/70 backdrop-blur-2xl p-5 rounded-3xl border border-pink-500/20 shadow-[0_8px_30px_rgba(0,0,0,0.3)] relative overflow-hidden group">
          ...
          <p className="text-3xl font-black text-pink-100 tracking-tight">{analytics.completedBookings}</p>
          <p className="text-[9px] font-extrabold uppercase tracking-widest text-pink-300/50 mt-1">Completed</p>
        </div>

        {/* Accept Rate with Progress Bar */}
        <div className="bg-[#2d1b4e]/70 backdrop-blur-2xl p-5 rounded-3xl border border-pink-500/20 shadow-[0_8px_30px_rgba(0,0,0,0.3)] relative overflow-hidden group col-span-2 sm:col-span-1">
          <div className="flex justify-between items-start mb-4">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-pink-400 to-pink-600 flex items-center justify-center text-white shadow-lg shadow-pink-500/30">
              <Activity className="w-5 h-5" />
            </div>
            <p className="text-2xl font-black text-pink-100">{analytics.acceptanceRate}%</p>
          </div>
          <p className="text-[9px] font-extrabold uppercase tracking-widest text-pink-300/50 mb-2">Acceptance Rate</p>
          <div className="w-full h-2 bg-[#1a0b2e] rounded-full overflow-hidden">
        ...
        <div className="bg-[#2d1b4e]/70 backdrop-blur-2xl p-5 rounded-3xl border border-pink-500/20 shadow-[0_8px_30px_rgba(0,0,0,0.3)] relative overflow-hidden group col-span-2 sm:col-span-1">
          <div className="flex justify-between items-start mb-4">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-purple-400 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-purple-500/30">
              <TrendingUp className="w-5 h-5" />
            </div>
            <p className="text-2xl font-black text-pink-100">{analytics.completionRate}%</p>
          </div>
          <p className="text-[9px] font-extrabold uppercase tracking-widest text-pink-300/50 mb-2">Completion Rate</p>
          <div className="w-full h-2 bg-[#1a0b2e] rounded-full overflow-hidden">
      ...
      <div className="bg-[#2d1b4e]/70 backdrop-blur-2xl p-6 rounded-3xl border border-pink-500/20 shadow-[0_8px_30px_rgba(0,0,0,0.3)] mb-6">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-pink-100 mb-6 flex items-center gap-2">
            ...
            return (
              <div key={item.month} className="flex-1 flex flex-col items-center gap-3 group cursor-pointer h-full justify-end">
                <div className="w-full relative flex justify-center h-full items-end">
                  <div className="opacity-0 group-hover:opacity-100 absolute -top-8 bg-[#2d1b4e] text-pink-100 text-[9px] font-bold px-2 py-1 rounded-lg transition-all transform group-hover:-translate-y-1 whitespace-nowrap z-10 shadow-lg border border-pink-500/20">
                    ₹{item.revenue.toLocaleString()}
                  </div>
                  <div
                    className={`w-full max-w-[24px] rounded-full transition-all duration-700 ease-out ${item.revenue > 0 ? 'bg-gradient-to-t from-pink-500 to-purple-400 shadow-[0_0_15px_rgba(236,72,153,0.4)] group-hover:shadow-[0_0_20px_rgba(236,72,153,0.6)]' : 'bg-[#1a0b2e]'}`}
                    style={{ height: `${Math.max(height, 8)}%` }}
                  />
                </div>
                <span className="text-[9px] font-extrabold text-pink-300/50 group-hover:text-pink-400 transition-colors uppercase">{item.month}</span>
              </div>
            );
          })}
        </div>
      ...
      {popularServices.length > 0 && (
        <div className="bg-[#2d1b4e]/70 backdrop-blur-2xl p-6 rounded-3xl border border-pink-500/20 shadow-[0_8px_30px_rgba(0,0,0,0.3)]">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-pink-100 mb-6 flex items-center gap-2">
            ...
            {popularServices.map((service) => (
              <div key={service.name}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-[9px] font-black text-pink-300 bg-pink-500/20 px-2 py-1 rounded-lg border border-pink-500/20">{service.count} SESSIONS</span>
                </div>
                <div className="h-3 bg-[#1a0b2e] rounded-full overflow-hidden p-0.5">
                  <div
